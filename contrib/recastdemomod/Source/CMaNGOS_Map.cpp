...
// itoa not defined on linux (at least on the test system. Defining it here)
#ifndef WIN32
...
void CMaNGOS_Map::scanFoldersForTiles()
{
    ...
    for (FileList::const_iterator itr = m_MapFiles.begin(); itr != m_MapFiles.end(); itr++)
    {
        unsigned int tileX = atoi((*itr).substr(3, 2).c_str());
        unsigned int tileY = atoi((*itr).substr(5, 2).c_str());
    ...
    for (FileList::const_iterator itr = m_VMapFiles.begin(); itr != m_VMapFiles.end(); itr++)
    {
        unsigned int tileX = atoi((*itr).substr(4, 2).c_str());
        unsigned int tileY = atoi((*itr).substr(7, 2).c_str());
